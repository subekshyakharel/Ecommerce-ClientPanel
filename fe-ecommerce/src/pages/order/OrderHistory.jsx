...
import { deleteOrderApi } from "../../features/order/orderApi";
...
const OrderHistory = () => {
  ...
  const visibleRows = myOrder.slice(
    page * rowsPerPage,
    page * rowsPerPage + rowsPerPage
  );

  const handleOnReview = async({_id, productId}) => {
    const orderData = {_id, productId}
    // console.log("review")
    dispatch(
      setModalContent({content: <LeaveReview orderData={orderData}/>, title:"Leave Review"})
    );
    dispatch(setmodalshow(true))
  };

  const handleOnCancelOrder = async({_id}) => {
    if(confirm("Are you sure you want to cancel the order?")){
      await deleteOrderApi(_id);
